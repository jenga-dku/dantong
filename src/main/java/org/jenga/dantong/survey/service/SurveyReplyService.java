...
public class SurveyReplyService {
    ...
    public List<SurveyUserReplyResponse> findUserReply(Long surveyId, Long userId) {
        ...
        List<SurveyReply> reply = items.stream().map(
            currItem -> surveyReplyRepository.findBySurveyItem_SurveyItemIdAndUserId(
                currItem.getSurveyItemId(), userId)).toList();
    ...
    public void createReply(List<SurveyReplyCreateRequest> request, Long userId) {
        ...
        request.forEach(currRequest -> {
            ...
            reply.setSurveyItem(
                surveyItemRepository.findBySurveyItemId(currRequest.getSurveyItemId()));
    ...
    @Transactional
    public void deleteReply(Long surveyId, Long userId) {
        User user = userRepository.findById(userId).orElseThrow(UserNotFoundException::new);
        Survey survey = surveyRepository.findById(surveyId)
            .orElseThrow(SurveyNotFoundException::new);
        List<SurveyItem> items = surveyItemRepository.findBySurvey(survey);

        List<SurveyReply> reply = items.stream().map(currItem -> {
                SurveyItem surveyItem = surveyItemRepository.findById(currItem.getSurveyItemId())
                    .orElseThrow(SurveyItemNotFoundException::new);
                return surveyReplyRepository.findSurveyReplyByUserAndSurveyItem(user, surveyItem)
                    .orElseThrow(
                        SurveyReplyNotFoundException::new);
            })
            .toList();
